...
- Severity: medium
- Status: RESOLVED
- Problem: docs/sprint-artifacts/atdd-tests-2.4.txt conteneva path root-level (packages/backend/...) che con npm test --workspaces veniva passato a Vitest backend come filtro non valido, causando falso fail del gate GREEN.
...
- Fix: aggiunto pre-check esplicito su fornitore.findFirst({ where: { partitaIva } }) in packages/backend/src/services/anagrafiche-service.ts prima della transaction, con ritorno PARTITA_IVA_EXISTS.
