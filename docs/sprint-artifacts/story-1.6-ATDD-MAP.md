...
- **Then** I receive HTTP `200` with `response.body.success` set to `true`, login with `NewPass2` returns `200`, and login with old password `Password1` returns `401` `INVALID_CREDENTIALS`
...
- **Then** I receive HTTP `400` with `error.code` set to `CURRENT_PASSWORD_INCORRECT` and exact message
- Test file: `packages/backend/src/__tests__/users-change-password.spec.ts`
