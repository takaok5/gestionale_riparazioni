...
- [ ] Task 5 (AC-1, AC-2, AC-3, AC-4): implement ATDD coverage in `packages/backend/src/__tests__/portal-documenti-download.atdd.spec.ts` for success (fattura/preventivo), forbidden cross-customer access, and unauthenticated request behavior (`401` + `UNAUTHORIZED` code + token error message).
