...
- [x] ATDD: estendere `packages/backend/src/__tests__/ordini-create-atdd.spec.ts` con scenari AC-1..AC-7 per `PATCH /api/ordini/:id/stato`, assert su HTTP code, stato risultante, messaggi errore e timestamp.
...
- Deviazione rispetto al task ATDD originale: invece di estendere `ordini-create-atdd.spec.ts`, e' stato introdotto `packages/backend/src/__tests__/ordini-stato-atdd.spec.ts` per isolare i casi AC-1..AC-7 dello stato ordine.
