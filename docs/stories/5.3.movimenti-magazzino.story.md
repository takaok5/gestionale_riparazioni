...
- [ ] ATDD backend: creare `packages/backend/src/__tests__/articoli-movimenti-atdd.spec.ts` con casi AC-1..AC-5 e verifiche esplicite su status `201/400`, messaggi errore e giacenza finale.
- [ ] Concurrency ATDD: in `packages/backend/src/__tests__/articoli-movimenti-atdd.spec.ts` aggiungere scenario con due chiamate parallele `SCARICO` (`Promise.all`) e assert che solo una richiesta passi, con giacenza finale coerente con una sola sottrazione.
