...
- [ ] Task 1 (AC-1, AC-2, AC-3, AC-4): aggiornare `packages/backend/src/routes/clienti.ts` per supportare `GET /api/clienti`, con parsing query (`page`, `limit`, `search`, `tipologia`) e mapping errori di validazione.
- [ ] Task 2 (AC-1, AC-2, AC-3, AC-4): introdurre in `packages/backend/src/services/anagrafiche-service.ts` il parser input lista clienti con default, validazioni e normalizzazione filtri.
- [ ] Task 3 (AC-1, AC-2, AC-3): implementare in `packages/backend/src/services/anagrafiche-service.ts` la lista clienti su test store e database con ordinamento deterministico e risposta `{ data, meta }`.
- [ ] Task 4 (AC-1, AC-2, AC-3, AC-4): registrare la route clienti aggiornata in `packages/backend/src/index.ts` mantenendo il prefix `/api/clienti`.
- [ ] Task 5 (AC-1, AC-2, AC-3, AC-4): estendere `packages/backend/src/__tests__/clienti-create-atdd.spec.ts` con scenari per paginazione, ricerca, filtro tipologia e sad path `tipologia=INVALID`.
- [ ] Task 6 (AC-1): riusare pattern di assert paginazione da `packages/backend/src/__tests__/audit-trail.spec.ts` per verificare `meta.page`, `meta.limit`, `meta.total`, `meta.totalPages` e `data.length`.
