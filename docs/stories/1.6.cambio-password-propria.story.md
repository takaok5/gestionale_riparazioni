...
**Then** I receive HTTP `200` and a subsequent `POST /api/auth/login` with `username="mario.rossi"` and `password="NewPass2"` returns HTTP `200`
...
**Then** I receive HTTP `400` with `response.body.error.message = "Current password is incorrect"`
...
**Then** I receive HTTP `400` with `response.body.error.code = "VALIDATION_ERROR"` and `response.body.error.details` containing `field="newPassword"`, `rule="password_policy"`, `min=8`, `requiresUppercase=true`, `requiresNumber=true`
