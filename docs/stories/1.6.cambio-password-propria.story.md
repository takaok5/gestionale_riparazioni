...
**Then** I receive HTTP `200` with `response.body.success = true`, a subsequent `POST /api/auth/login` with `username="mario.rossi"` and `password="NewPass2"` returns HTTP `200`, and login with old password `Password1` returns HTTP `401` with `error.code = "INVALID_CREDENTIALS"`
...
**Then** I receive HTTP `400` with `response.body.error.code = "CURRENT_PASSWORD_INCORRECT"` and `response.body.error.message = "Current password is incorrect"`
...
**Then** I receive HTTP `400` with `response.body.error.code = "VALIDATION_ERROR"`, `response.body.error.message = "Payload non valido"`, and `response.body.error.details = { field: "newPassword", rule: "password_policy", min: 8, requiresUppercase: true, requiresNumber: true }`
...
- [x] Task 3 (AC-3): introdurre in `packages/backend/src/services/users-service.ts` la validazione `newPassword` (`min >= 8`, almeno 1 uppercase, almeno 1 numero) e restituire `VALIDATION_ERROR` con `details` strutturati.
